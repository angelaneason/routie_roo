      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (ctx.user.enableSmartRouting === 1) {
          ...
          for (const day of removedDays) {
            ...
            // Find routes for this day - match by scheduledDate instead of name to handle route holder prefixes
            const routesToUpdate = await db.select()
              .from(routes)
              .where(and(
                eq(routes.userId, ctx.user.id),
                eq(routes.isArchived, false),
                eq(routes.isAutoGenerated, true),
                sql`DATE(${routes.scheduledDate}) = DATE(${dayDate.toISOString().split('T')[0]})`
              ));

            for (const route of routesToUpdate) {
              // Remove this contact's waypoint from the route
              ...
              // Check if route is incomplete and has no remaining waypoints
              if (route.isAutoGenerated && !route.completedAt) {
                // Get remaining waypoints (excluding starting point at position 0)
                const remainingWaypoints = await db.select()
                  .from(routeWaypoints)
                  .where(and(
                    eq(routeWaypoints.routeId, route.id),
                    sql`${routeWaypoints.position} > 0`
                  ));
                
                // If no waypoints remain (only starting point), delete the route
                if (remainingWaypoints.length === 0) {
                  // Delete all waypoints (including starting point)
                  await db.delete(routeWaypoints)
                    .where(eq(routeWaypoints.routeId, route.id));
